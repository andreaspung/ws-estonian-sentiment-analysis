...
if __name__ == '__main__':
    ...
    print(f"Supervised BERT more finetuning {cache_name}")
    ...
    search_space = {
        "batch_size": [16, 32],
        "optimizer_lr": [5e-6, 1e-5, 3e-5, 5e-5, 1e-4],
    ...
    for i in range(5):
        ...
        model.fit(
            ...
            evaluation_step=100,
            metric='acc',
            #patience=100, #and we early stop the training processbased on the evaluation metric
            # values on validation set
            device=device
        )

        model.save(dataset_path + data + f"/best_bert_supervised_{cache_name}_model_{i}.pkl")
